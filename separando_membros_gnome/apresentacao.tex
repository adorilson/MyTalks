...
\subtitle{Lições aprendidas com hacking no Damned Lies}
...
{
    \begin{itemize}
        \item "Pagamento", retribuição
        \item Necessidade
        \item Aprendizado
        \item Diversão, realização
        \item ...
    \end{itemize}
}
...
\frame
{
    ...
    \begin{columns}
        ...
        \end{figure}
    \end{columns}
    {\tiny Fotos por: Krix Apolinário (\url{http://blog.krix.com.br/2010/11/iv-ensl-e-vii-forum-gnome-\%e2\%80\%93-parte-22/})}
}
\frame
{
    \frametitle{Criação do bug em http://bugzilla.gnome.org => http://vai.la/2aqc}
    \begin{figure}[t]
...
\frame
{
    \frametitle{Cultive o desapego ao código}
    \begin{figure}[t]
...
\frame
{
    \frametitle{Participe da governança}
    \begin{figure}[t]
...
\frame
{
    \frametitle{Software livre é bom para você}
    \begin{figure}[t]
...
\frame{
	\frametitle{GNOME Goals (Slide bônus)}
	\begin{figure}[t]
		\includegraphics[scale=0.3]{img/gnome_goals}
	\end{figure}
}

Colaboracao e Open Source dentro da empresa - Guilherme Chapiewski
http://vai.la/2uIi
